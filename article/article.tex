...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/development/mcp-generator-architecture.jpg}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// Extração simplificada do processo de parsing OpenAPI}
\FunctionTok{parseOpenAPISpec}\NormalTok{(spec}\OperatorTok{:}\NormalTok{ OpenAPIV3}\OperatorTok{.}\AttributeTok{Document}\NormalTok{)}\OperatorTok{:} \BuiltInTok{Map}\OperatorTok{\textless{}}\DataTypeTok{string}\OperatorTok{,}\NormalTok{ Tool}\OperatorTok{\textgreater{}}\NormalTok{ \{}
  \KeywordTok{const}\NormalTok{ tools }\OperatorTok{=} \KeywordTok{new} \BuiltInTok{Map}\OperatorTok{\textless{}}\DataTypeTok{string}\OperatorTok{,}\NormalTok{ Tool}\OperatorTok{\textgreater{}}\NormalTok{()}
  
  \CommentTok{// Converter cada caminho OpenAPI em uma ferramenta MCP}
  \ControlFlowTok{for}\NormalTok{ (}\KeywordTok{const}\NormalTok{ [path}\OperatorTok{,}\NormalTok{ pathItem] }\KeywordTok{of} \BuiltInTok{Object}\OperatorTok{.}\FunctionTok{entries}\NormalTok{(spec}\OperatorTok{.}\AttributeTok{paths}\NormalTok{)) \{}
    \ControlFlowTok{for}\NormalTok{ (}\KeywordTok{const}\NormalTok{ [method}\OperatorTok{,}\NormalTok{ operation] }\KeywordTok{of} \BuiltInTok{Object}\OperatorTok{.}\FunctionTok{entries}\NormalTok{(pathItem)) \{}
      \KeywordTok{const}\NormalTok{ op }\OperatorTok{=}\NormalTok{ operation }\ImportTok{as}\NormalTok{ OpenAPIV3}\OperatorTok{.}\AttributeTok{OperationObject}
      \KeywordTok{const}\NormalTok{ toolId }\OperatorTok{=} \VerbatimStringTok{\textasciigrave{}}\SpecialCharTok{$\{}\NormalTok{method}\OperatorTok{.}\FunctionTok{toUpperCase}\NormalTok{()}\SpecialCharTok{\}}\VerbatimStringTok{{-}}\SpecialCharTok{$\{}\NormalTok{cleanPath}\SpecialCharTok{\}}\VerbatimStringTok{\textasciigrave{}}
      
      \CommentTok{// Estrutura da ferramenta MCP}
      \KeywordTok{const}\NormalTok{ tool}\OperatorTok{:}\NormalTok{ Tool }\OperatorTok{=}\NormalTok{ \{}
\NormalTok{        name}\OperatorTok{:} \KeywordTok{this}\OperatorTok{.}\FunctionTok{abbreviateOperationId}\NormalTok{(op}\OperatorTok{.}\AttributeTok{operationId}\NormalTok{)}\OperatorTok{,}
\NormalTok{        description}\OperatorTok{:}\NormalTok{ op}\OperatorTok{.}\AttributeTok{description} \OperatorTok{||} \VerbatimStringTok{\textasciigrave{}Make a }\SpecialCharTok{$\{}\NormalTok{method}\SpecialCharTok{\}}\VerbatimStringTok{ request to }\SpecialCharTok{$\{}\NormalTok{path}\SpecialCharTok{\}}\VerbatimStringTok{\textasciigrave{}}\OperatorTok{,}
\NormalTok{        inputSchema}\OperatorTok{:}\NormalTok{ \{}
\NormalTok{          type}\OperatorTok{:} \StringTok{"object"}\OperatorTok{,}
\NormalTok{          properties}\OperatorTok{:}\NormalTok{ \{\}}\OperatorTok{,}
