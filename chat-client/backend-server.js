    ...
    setupRoutes() {
        ...
        this.app.post('/mcp/initialize', async (req, res) => {
            try {
                ...
                const serverUrl = mcpServerUrl || config.mcp.defaultServerUrl;

                try {
                    const sessionData = await this.mcpClientManager.createSession(serverUrl);
                    ...
                    res.json({
                        ...
                        capabilities: {},
                        serverUrl: serverUrl,
                        error: `MCP server unavailable: ${mcpError.message}`,
                        fallbackMode: true
                    });
                }
            } catch (error) {
